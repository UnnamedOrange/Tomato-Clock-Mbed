...
private:
    ...
    sys_clock::time_point m_current_start_time{}; // Start time of the current period.
...
private:
    ...
    void play_audio(u8 index) {
        switch (index) {
        case 0: m_audio.push_beats(_song_1); break;
        case 1: m_audio.push_beats(_song_2); break;
        }
    }
...
private:
    ...
    void gui_show_message(state_t index) {
        if (index == state_t::work)
            Gui_DrawFont_GBK16(36, 62, BLUE, WHITE, reinterpret_cast<const u8*>("Working"));
        else
            Gui_DrawFont_GBK16(36, 62, BLUE, WHITE, reinterpret_cast<const u8*>("Resting"));
    }
    u8 TX(u8 n) {
        if (n < 5)
            return 32 * (n - 1);
    ...
    void transfer_state() {
        ...
        m_current_duration = m_state == state_t::work ? _default_duration_work : _default_duration_rest;
    }
    ...
    void draw() {
        ...
        if (m_state_drawing != state_drawing_t::idle) {
            show_pic(m_state_drawing == state_drawing_t::red_tomato ? Red_tomato : Green_tomato, TX(m_num_tomatoes),
                     TY(m_num_tomatoes), TX(m_num_tomatoes) + 30, TY(m_num_tomatoes) + 32);
